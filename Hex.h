...
struct Hex { // Axial storage, cube constructor
    //const int q_, r_;
    int q_, r_;
    Hex(int q=0, int r=0, int s=0): q_(q), r_(r) {
        assert (q + r + s == 0);
    }
    ...
    inline void setAxial(int q, int r, int s)
    { q_ = q;
      r_ = r;
      assert (q + r + s == 0);
    }

    unsigned long hexhashWorkaround() const {
    //return (size_t)(pt.x*100 + pt.y);
    //hash<int> int_hash;
    //size_t hq = q_;
    //size_t hr = r_;
    return q_ ^ (r_ + 0x9e3779b9 + (q_ << 6) + (q_ >> 2));
};

    public:
    bool operator == (const Hex& h) const { return ((q_ == h.get_q()) && (r_ == h.get_r())); }

    // note: this is done for A* algorithm and priority queue as Hex can NOT be compared
    bool operator<(Hex const &h) const { 
        //return (hexhashWorkaround() < h.hexhashWorkaround()); 
        //return int((std::abs(q_) + std::abs(r_) + std::abs(-q_-r_)) / 2) < int((std::abs(h.get_q()) + std::abs(h.get_r()) + std::abs(h.get_s())) / 2);;
        return true;
        }
